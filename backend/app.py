...
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=["http://127.0.0.1:5501", "http://localhost:5501", "*"],  # Added * for testing
#     allow_methods=["*"],
#     allow_headers=["*"],
# )
...
#         # WebSocket upgrades skip header auth (handled separately if needed)
...
#             # In debug mode, allow all requests but warn if key is wrong/missing
#             if key != API_KEY:
...
#     data_str = str(data.get("data", "BLOCK"))[:512]   # cap payload field size
#     last     = blockchain[-1]
...
#                     try:
#                         ipaddress.ip_address(line)   # validate before storing
#                         new_ips.add(line)
#                     except ValueError:
#                         pass
#             with THREAT_LOCK:
...
#     # Ping — runs in thread pool to avoid blocking event loop
#     try:
...
#     # DNS resolution — blocking, run in thread pool
#     try:
...
# class GossipEngine:
#     TICK_INTERVAL = 2.0   # seconds between gossip rounds
#     TRAVEL_DELAY  = 0.8   # seconds a packet travels before resolution
...
#         # Store the running event loop so threads can schedule coroutines onto it
#         self._loop = asyncio.get_event_loop()
#         await self._send(ws, {
#             "type":           "sync",
#             "nodes":          [{"lbl": n["lbl"], "state": n["state"], "score": n["score"]}
...
#         self.nodes      = {}
#         self.node_index = []
#         self.edge_hops  = {}          # reset hop counters for new simulation
...
#             action = "added"
#             # Auto-quarantine if the infected node is now fully isolated
...
#             # Collect all (node, peer) spread candidates first,
#             # then process them — avoids mutating state mid-iteration
#             candidates = []
...
#                 # Re-validate after travel delay
...
#             # Schedule coroutine onto the event loop from this background thread
...
#                 gossip.nodes          = {}
#                 gossip.edge_hops      = {}
#                 gossip.firewall_edges = set()
#                 gossip.total_reached  = 0
#                 gossip.total_immune   = 0
#                 gossip.total_quarantine = 0
...
# # ── Global crypto state (in production, use proper key management) ──
# _crypto_state = {
...
#     # Accept both 'text' (from JS) and 'value' (legacy)
...
#     """Sign a message with RSA-2048 private key."""
...
#     # Accept both 'text' (from JS) and 'plaintext' (legacy)
...
#     # Store for later decryption test
