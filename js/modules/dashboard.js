...
//       existing.cat   = r.cat;
//     } else {
//       topIps.push({ ip: val, count: 1, cat, score: r.score });
//     }
...
//       <th>IP Address</th><th>Count</th><th>Category</th><th>Score</th>
//     </tr></thead><tbody>
...
//   // Destroy old instances if they exist (e.g. theme toggle rebuild)
...
//   if (!dc || !tc) {
//     // Canvas not in DOM yet — retry once layout is ready
//     requestAnimationFrame(buildCharts);
//     return;
//   }

//   // Force canvas to have explicit dimensions so Chart.js can render
...
//   localStorage.setItem('sentraDashData', JSON.stringify({ threatCounts, timeData, topIps }));

//   // If charts haven't been built yet (page not visible), skip
//   if (!dashChartsBuilt || !distChart || !timeChart) return;
...
//   timeChart.data.datasets[0].data = [...timeData.malware];
//   timeChart.data.datasets[1].data = [...timeData.phishing];
//   timeChart.update();

//   // Update stat cards
//   const total = threatCounts.malware + threatCounts.phishing +
//                 threatCounts.suspicious + threatCounts.safe +
//                 threatCounts.unknown;
...
//   if (malwareEl  && threatCounts.malware > 0)    malwareEl.textContent  = 342 + threatCounts.malware;
//   if (phishingEl && threatCounts.phishing > 0)   phishingEl.textContent = 189 + threatCounts.phishing;
// }
...
//   // Build charts — dashboard page is active at this point
//   buildCharts();
//   // Push saved data into charts immediately
//   updateDashboardCharts();
// }


// document.addEventListener('DOMContentLoaded', () => {
//   // Patch gotoPage (defined in router.js) to rebuild charts
//   // whenever the user switches to the dashboard page.
//   const _origGoto = window.gotoPage;
//   window.gotoPage = function(page, el) {
//     if (typeof _origGoto === 'function') _origGoto(page, el);
//     if (page === 'dashboard') {
//       // Small delay so the page is visible before Chart.js measures canvas
//       setTimeout(() => {
//         buildCharts();
//         updateDashboardCharts();
//         buildTopIpTable();
